...
const generateExpenseInsights = async (expenses) => {
  ...
  try {
    ...
    const prompt = `
      ...
      Important: 
      - Use the Indian Rupee symbol (₹) for any currency values in the response and make sure not to include any currency symbols in the numeric values of the JSON.
    ...
    // Parse JSON response - with improved error handling for markdown code blocks
    try {
      ...
      try {
        ...
        // If no code blocks found, throw the original error
